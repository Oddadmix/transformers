...
class Qwen3ASRThinkerTextRotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(self, config, device=None, seq_len=None, **kwargs):
        return _compute_default_rope_parameters(config, device, seq_len, **kwargs)
...
class Qwen3ASRThinkerTextAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.q_norm = Qwen3ASRThinkerTextRMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.k_norm = Qwen3ASRThinkerTextRMSNorm(self.head_dim, eps=config.rms_norm_eps)
...
# ============================================================================
# Thinker For Conditional Generation
# ============================================================================

@auto_docstring(
    custom_intro="""
    The Qwen3ASRThinker model which consists of an audio backbone and a language model.
    """
)
class Qwen3ASRThinkerForConditionalGeneration(Qwen3ASRPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def get_audio_features(
        ...
        # audio encoder does not support batch inference to keep precision
        audio_features = []
